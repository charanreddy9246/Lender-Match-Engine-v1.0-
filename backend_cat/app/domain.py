...
def calculate_customer_foir_pct(monthly_income: float, monthly_obligations: float) -> float:
    ...
    Not built through the generic rule engine (EligibilityRuleDef/
    _rule_satisfied) on purpose — that engine only compares a bank's stored
    value against something the borrower answered directly (like CIBIL
    score). This value isn't a direct answer; it's calculated from two other
    answers (income and obligations), which the generic engine has no way
    to express.
    """
    if monthly_income <= 0:
        return 0.0
...
def get_bank_interest_rate_upper_pct(product: HomeLoanProduct) -> float | None:
    ...
    "interest_rate_upper_pct" fact — only set for banks whose source actually
    published a range (e.g. "7.25%-8.70%"), not for banks that only ever
    published a single "X% onwards" figure. None means no real upper bound
    exists to show, not that one wasn't loaded yet.
    """
